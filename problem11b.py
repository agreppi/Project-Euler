...
for i in range(20):
    for j in range(16):
        ...
        if prod > max_prod: 
            print("1) Fila {}, columna {} , prod {}".format(i, j, prod))
            max_prod = prod
        # up/down products
        prod = M[j][i]*M[j+1][i]*M[j+2][i]*M[j+3][i]
        if prod > max_prod:
            print("2) Fila {}, columna {} , prod {}".format(j, i, prod))
            max_prod = prod
...
for i in range(16):
    for j in range(16):
        ...
        if prod > max_prod:
            print("3) Fila {}, columna {} , prod {}".format(i, j, prod))
            max_prod = prod
for i in range(3,20):
    for j in range(16):
        prod = M[i][j]*M[i-1][j+1]*M[i-2][j+2]*M[i-3][j+3]
        if prod > max_prod: 
            print("4) Fila {}, columna {} , prod {}".format(i, j, prod))
            max_prod = prod
